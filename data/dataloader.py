...
class FlowLoader(Dataset):
    def __init__(
        ...
        for v in self.data_seq:
            ...
            samples = [
                os.path.join(x.split("/")[-2], x.split("/")[-1]) for x in samples
            ]
            grouped_samples = getFrameGroupsList(
                samples, self.frames, self.frame_overlap, self.stride
            )
    ...
    def readSingleSmpl(self, smpl_name, amodal=True):
        ...
        for gap, _gap in zip(self.gaps, self.neg_gaps):
            flow_dir = (
                os.path.join(self.data_dir[0], smpl_name)
                .replace("gap1", gap)
                .replace(".jpg", ".flo")
            )
        ...
        if self.data_dir[2] is None:  # No GT provided. Empty array output.
            ...
            self.dataset == "FBMS"
        ):  # If temporally sparse GTs are provided, empty arrays will be added to complete the sequence
            seq_name = os.path.dirname(smpl_name)
            samples = sorted(
                glob.glob(os.path.join(self.data_dir[2], seq_name, "*.png"))
            )
            gt_names = [
                os.path.join(x.split("/")[-2], x.split("/")[-1]) for x in samples
            ]
            ...
            gt_res = refseg.shape
            if (
                smpl_name.replace(".jpg", ".png") in gt_names
            ):  # check if GT exists for particular frame
                gt_dir = os.path.join(self.data_dir[2], smpl_name).replace(
                    ".jpg", ".png"
                )
                seg = readSeg(gt_dir)
            else:
            ...
            if self.amodal:
                seg_gt = readAmodalSeg(
                    gt_dir.replace("Annotations", "AmodalAnnotations"),
                    resolution=self.gt_res,
                    out_channels=self.out_channels,
                )
            else:
                seg = readSeg(gt_dir)
                seg_gt = processMultiSeg(
                    seg, self.gt_res, self.out_channels, self.dataset
                )
    ...
    def __getitem__(self, idx):
        ...
        img_dir = [
            os.path.join(self.data_dir[2], i).replace(".jpg", ".png").split("/")[-2:]
            for i in smpl_names
        ]
