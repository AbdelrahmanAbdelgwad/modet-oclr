...
def main(args):
    ...
    trn_dataset, val_dataset, resolution, in_channels, out_channels = cg.setup_dataset(
        args
    )
    ...
    while it < num_it:
        for i, sample in enumerate(trn_loader):
            ...
            mask_am_raw = torch.nan_to_num(mask_am_raw, nan=0.0, posinf=0.0, neginf=0.0)
            order_raw = torch.nan_to_num(order_raw, nan=0.0, posinf=0.0, neginf=0.0)

            mask_am_raw, order_raw = ut.hungarian_matcher(
                mask_am_raw, gt_am, [mask_am_raw, order_raw]
            )  # hungarian matching

            # amodal losses
            loss_amodal_bce = weight_amodal_bce * ut.criterion_amodal_bce(
                mask_am_raw, gt_am
            )
            loss_amodal_bound = weight_amodal_bound * ut.criterion_amodal_bound(
                mask_am_raw, gt_am
            )
            ...
            if it % log_freq == 0:
                ...
                writer.add_scalar(
                    "Loss/amodal_bce", loss_amodal_bce.detach().cpu().numpy(), it
                )
                writer.add_scalar(
                    "Loss/amodal_bound", loss_amodal_bound.detach().cpu().numpy(), it
                )
            ...
            if it % eval_freq == 0:
                ...
                filename = os.path.join(
                    modelPath,
                    "ckpt_{}-(modal_{})-(amodal_{}).pth".format(
                        it, np.round(meaniou_m, 3), np.round(meaniou_am, 3)
                    ),
                )
                torch.save(
                    {
            ...
            if (
                it % decay_step == 0 and it // decay_step <= 9
            ):  # decaying steps (lr divided by 2)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_train_steps", type=int, default=60)
    ...
    parser.add_argument(
        ...
        type=str,
        default="Syn",
        choices=["Syn", "DAVIS17m", "DAVIS16", "Segtrack", "FBMS", "MoCA"],
    )
    parser.add_argument(
        "--gaps", type=str, default="1,-1"
    )  # Two flow gaps inputs, input string should not include space in-between.
    ...
    )  # For training, keep this as None
