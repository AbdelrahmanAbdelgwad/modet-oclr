...
class BTNTransformerEnc(nn.Module):
    def __init__(self, btn_features, num_layers, num_heads):
        ...
        self.transformer_encoder = TransEncoder(
            btn_features, num_layers, num_heads, dim_feedforward=512
        )
...
class BTNTransformerDec(nn.Module):
    def __init__(self, btn_features, num_layers, num_heads, num_query):
        ...
        self.transformer_decoder = TransDecoder(
            btn_features, num_layers, num_heads, dim_feedforward=512
        )
    ...
    def forward(self, btn):
        # b t h w c
        b, t, h, w, c = btn.size()
        btn = einops.rearrange(
            btn, "b t h w c -> b (t h w) c"
        )  # Flatten spacetime dimensions. #to: b (t h w) c
        embed = self.motion_embed(
            torch.arange(0, self.num_query).expand(b, self.num_query).to(self.device)
        )  # to: b q c
...
class AttnMap(nn.Module):
    def __init__(self, btn_features, num_heads, num_query):
        ...
        self.mlp = nn.Sequential(
            nn.Linear(num_heads * num_query, btn_features), nn.ReLU(inplace=True)
        )

    def forward(self, btn_dec, btn_enc):
        b, t, h, w, _ = btn_enc.size()
        btn_enc = einops.rearrange(
            btn_enc, "b t h w (g c) -> (b g) t h w c", g=self.heads
        )
        btn_dec = einops.rearrange(
            btn_dec, "b (q t) (g c) -> (b g) q t c", t=t, g=self.heads
        )
        btn_out = btn_dec[:, :, :, None, None, :] * btn_enc[:, None, :, :, :, :]
        btn_out = torch.sum(btn_out, 5)  # to: (b g) q t h w
        btn_out = einops.rearrange(
            btn_out, "(b g) q t h w -> b (t h w) (q g)", g=self.heads
        )
...
class OCLR(nn.Module):
    def __init__(
        ...
        self.transformer_dec = BTNTransformerDec(
            btn_features, num_layers, num_heads, num_query
        )
    ...
    def forward(self, flow):
        ...
        print(out_order.size())
        out_order = out_order[:, None].expand(
            b, t, c
        )  # to: b t c, which copies the same global ordering for all frames.
